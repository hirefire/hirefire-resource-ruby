...
- The dispatcher is now fork-aware: a forked worker (e.g. Puma cluster with `preload_app!`) detects that the inherited dispatcher thread did not survive the fork and starts a fresh one on the first request, instead of silently never dispatching. Each child also re-issues its own lease identity and drops any inherited grant, so forked workers do not all poll the job queues under the parent's identity.
