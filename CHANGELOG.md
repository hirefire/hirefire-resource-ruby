...
- YARD documentation on the core public API (entrypoint, configuration, dispatcher, collectors, middleware, errors), extending the coverage the queue macros already had.
...
- Dispatcher loop threads bind to a start generation, so a hung loop that outlives `stop`'s join cannot resume work after a later `start` (matching the Python client).
