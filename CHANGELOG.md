...
- Job-queue **working** count (`job_queue_working`) for Solid Queue (`ClaimedExecution`), Delayed Job (`locked_at` set), Que (advisory lock / `pg_locks`), Good Job (`performed_at` set, unfinished), and Queue Classic (`locked_at` set), matching Sidekiq. Plan path samples nested strategy **`wrk`** for each of these adapters next to `jql`/`jqs` (same queues, unconditional). Not folded into waiting JQL/JQS.
...
- Sidekiq client schedule/retry due walks use a process-local rank cache scoped to one `Dispatcher#sample_job_queues` wave so many named JQL/JQS samplers amortize one ascending walk per set per sample. The dispatcher opens the wave only via generic `Plan.around_job_queue_sample` (every macro's `before_sample_job_queues` / `after_sample_job_queues`, no-op by default). Sidekiq's hooks drive DueCache. Outside a sample wave (console or one-off macros), each call walks cold. JQL still early-stops on the first matching due in the requested union. JQS serves due counts only after the due region is complete for that fill (except a call-local `max_scheduled` cap). Corrupt ZSET members are skipped (cursor advances) instead of aborting the sample. Live lists and `server: true` Lua stay uncached. Macro reinit after fork/abandon goes through `Plan.reinit_macros_after_fork!` (same sites as the metric buffer).
- `HireFire::Macro::Que` JQL and JQS are waiting-only: due unfinished/unexpired jobs that are not session advisory-locked (`pg_locks` anti-join matching Que `job_stats`). Locked (working) jobs are no longer counted in size or latency. No `skip_working` flag (unlike Sidekiq). Due rows with `error_count > 0` still count (do not copy Que AR `ready`). Deprecated `.queue` is aligned via `job_queue_size`.
