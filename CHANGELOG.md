...
* Declaring a second `:rqt`/`:rpm` dyno now raises (request metrics come from this process's own http traffic, so only one http dyno can exist per process), and duplicate-name detection is case-insensitive, matching the identity gates.
